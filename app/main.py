...
const tabNames = {overview:["Command Overview","Memory Constellation"],vault:["Memory Vault","Recall Archive"],agents:["Agent Registry","Channel Topology"],ops:["Operations","RTK · Sessions · Telemetry"],console:["Neural Console","Direct Recall Interface"]};
...
function switchTab(tab){document.querySelectorAll('.tabs').forEach(e=>e.classList.remove('active'));document.getElementById('tab-'+tab).classList.add('active');document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));document.getElementById('tab-eyebrow').textContent=tabNames[tab][0];document.getElementById('tab-title').textContent=tabNames[tab][1];if(tab==='vault')loadMemories();if(tab==='agents')renderAgentCards();if(tab==='ops')loadOps();}
