...
function labList(items){return (items||[]).map((m,i)=>`${i+1}. [${m.memory_type}] score=${Number(m.score||0).toFixed(3)} imp=${Math.round((m.importance||0)*100)}%\n${m.content}`).join('\n\n')||'(no results)';}
async function runRecallLab(){const q=document.getElementById('lab-query').value.trim();if(!q)return;['vector','lexical','graph','temporal'].forEach(m=>document.getElementById('lab-'+m).textContent='running…');document.getElementById('lab-fused').textContent='fusing…';try{const d=await apiFetch('/v1/memory/recall/debug',{method:'POST',body:JSON.stringify({query:q,agent_id:state.agent,limit:8,search_modes:['vector','lexical','graph','temporal']})});['vector','lexical','graph','temporal'].forEach(m=>document.getElementById('lab-'+m).textContent=labList(d.per_mode?.[m]||[]));document.getElementById('lab-fused').textContent=`QUERY: ${d.query}\nEXPANDED: ${d.expanded_query}\nFUSION: ${d.fusion}\nMODE COUNTS: ${JSON.stringify(d.explanation?.mode_counts||{})}\n\nFUSED:\n${labList(d.fused)}\n\nRERANKED:\n${labList(d.reranked)}`;}catch(e){document.getElementById('lab-fused').textContent='Recall lab failed: '+e.message;}}
