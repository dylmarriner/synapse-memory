...
from app.routers.obelisk import router as obelisk_router
...
app.include_router(obelisk_router,     prefix="/v1/obelisk",     tags=["obelisk"],     dependencies=[Depends(_verify_key)])
...
<div class="app">
  ...
  <main class="main shell">
    ...
    <section id="tab-ops" class="tabs console"><div class="statGrid"><div class="stat"><div class="lbl">Sessions</div><div class="val" id="o-sessions">…</div><div class="sig">RAW/CTX</div></div><div class="stat"><div class="lbl">Messages</div><div class="val" id="o-messages">…</div><div class="sig">EVENT/LOG</div></div><div class="stat"><div class="lbl">Obelisk Saved</div><div class="val" id="o-obelisk-tokens">…</div><div class="sig">TOKENS</div></div><div class="stat"><div class="lbl">Obelisk Failures</div><div class="val" id="o-obelisk-fail">…</div><div class="sig">24H</div></div></div><div class="overviewGrid"><div class="panel"><h3>Obelisk By Agent</h3><div class="consoleOut" id="ops-obelisk-agents">Loading…</div></div><div class="panel"><h3>Recent Events</h3><div class="consoleOut" id="ops-events">Loading…</div></div></div></section>
...
async function loadOps(){try{const [m,r]=await Promise.all([apiFetch('/v1/admin/metrics'),apiFetch('/v1/admin/obelisk/summary')]);document.getElementById('o-sessions').textContent=m.totals?.sessions??0;document.getElementById('o-messages').textContent=m.totals?.messages??0;document.getElementById('o-obelisk-tokens').textContent=r.tokens_saved_estimate??0;document.getElementById('o-obelisk-fail').textContent=r.failures??0;document.getElementById('ops-obelisk-agents').textContent=(r.by_agent||[]).map(a=>`${a.agent_id}: ${a.tokens_saved_estimate} tokens · ${a.count} cmds · ${a.failures} failures`).join('\n')||'No Obelisk telemetry yet.';document.getElementById('ops-events').textContent=(m.recent_events||[]).slice(0,12).map(e=>`${fmtDate(e.created_at)} ${e.action} ${e.actor||''}`).join('\n')||'No recent events.';}catch(e){document.getElementById('ops-events').textContent='Operations load failed: '+e.message;}}
