...
<div class="app">
  ...
  <main class="main shell">
    ...
    <section id="tab-recalllab" class="tabs console"><div class="consoleForm"><input class="field" id="lab-query" placeholder="Compare recall modes…"><button class="action" onclick="runRecallLab()">Run Lab</button></div><div class="overviewGrid"><div class="panel"><h3>Vector</h3><div class="consoleOut" id="lab-vector">—</div></div><div class="panel"><h3>Lexical</h3><div class="consoleOut" id="lab-lexical">—</div></div><div class="panel"><h3>Graph</h3><div class="consoleOut" id="lab-graph">—</div></div><div class="panel"><h3>Temporal</h3><div class="consoleOut" id="lab-temporal">—</div></div></div><div class="panel"><h3>Fused / Reranked</h3><div class="consoleOut" id="lab-fused">NEXUS:// recall lab ready</div></div></section>
...
function labList(items){return (items||[]).map((m,i)=>`<div class="mem-card"><div class="mem-header"><span class="type-badge ${typeBadgeClass(m.memory_type)}">${escHtml(m.memory_type)}</span><span class="mem-meta">#${i+1} score=${Number(m.score||0).toFixed(3)} imp=${Math.round((m.importance||0)*100)}%</span></div><div class="mem-content">${escHtml(m.content)}</div><div class="mem-footer"><button class="del-btn" onclick="feedbackMemory('${m.id}','confirm')">Useful ✓</button><button class="del-btn" onclick="feedbackMemory('${m.id}','contradict')">Not useful ⚠</button><button class="del-btn" onclick="navigator.clipboard?.writeText('${m.id}')">Copy ID</button></div></div>`).join('')||'<div class="empty">No results</div>';}
...
async function runRecallLab(){const q=document.getElementById('lab-query').value.trim();if(!q)return;['vector','lexical','graph','temporal'].forEach(m=>document.getElementById('lab-'+m).innerHTML='<div class="loading">running…</div>');document.getElementById('lab-fused').textContent='fusing…';try{const d=await apiFetch('/v1/memory/recall/debug',{method:'POST',body:JSON.stringify({query:q,agent_id:state.agent,limit:8,search_modes:['vector','lexical','graph','temporal']})});['vector','lexical','graph','temporal'].forEach(m=>document.getElementById('lab-'+m).innerHTML=labList(d.per_mode?.[m]||[]));document.getElementById('lab-fused').innerHTML=`<div class="consoleOut">QUERY: ${escHtml(d.query)}\nEXPANDED: ${escHtml(d.expanded_query)}\nFUSION: ${escHtml(d.fusion)}\nMODE COUNTS: ${escHtml(JSON.stringify(d.explanation?.mode_counts||{}))}</div><h3>Fused</h3>${labList(d.fused)}<h3>Reranked</h3>${labList(d.reranked)}`;}catch(e){document.getElementById('lab-fused').textContent='Recall lab failed: '+e.message;}}
