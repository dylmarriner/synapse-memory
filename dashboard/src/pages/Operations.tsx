...
import { fetchMetrics, fetchObeliskSummary } from '../api/nexus';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';

export default function Operations() {
  const { data: metrics } = useQuery({ queryKey: ['metrics'], queryFn: fetchMetrics, refetchInterval: 15000 });
  const { data: obelisk } = useQuery({ queryKey: ['obelisk-summary'], queryFn: fetchObeliskSummary, refetchInterval: 15000 });

  const obelisks = metrics?.obelisk;
    ...
    <>
      <div className="flex items-center gap-4 mb-4">
        <div>
          ...
          <h2 className="text-3xl m-0 mt-1">Obelisk · Sessions · Telemetry</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Obelisk Saved', value: obelisks?.tokens_saved_estimate ?? '…', sig: 'TOKENS' },
          { label: 'Obelisk Failures', value: obelisks?.failed_24h ?? '…', sig: '24H' },
        ].map((s) => (
      ...
      <div className="grid grid-cols-2 gap-4 flex-1 min-h-0 overflow-auto">
        ...
        <div className="border border-[rgba(102,252,241,.2)] bg-[var(--color-panel)] p-4">
          <h3 className="text-[.82rem] text-[var(--color-cyan)] uppercase tracking-[.16em] m-0 mb-3">Obelisk By Agent</h3>
          {obelisk?.by_agent?.length ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={obelisk.by_agent}>
            ...
            <div className="text-xs text-[var(--color-dim)]">No Obelisk telemetry yet.</div>
          )}
          <div className="mt-2 space-y-1 text-xs text-[var(--color-muted)]">
            {obelisk?.by_agent?.slice(0, 5).map((a) => (
